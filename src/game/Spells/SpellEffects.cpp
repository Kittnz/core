    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                {
                    if (m_caster->GetTypeId() != TYPEID_PLAYER ||
                        m_CastItem && m_CastItem->GetEntry() != 6657) /* To allow the use of custom scripted behaviours */
                        return;
                ...
                {
                    if (m_caster->GetTypeId() != TYPEID_PLAYER ||
                        m_CastItem && m_CastItem->GetEntry() != 8529) /* To allow the use of custom scripted behaviours */
                        return;
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 25755:
                {
                    unitTarget->CastSpell(m_caster, 26639, true);
                    return;
                }
...
void Spell::EffectFeedPet(SpellEffectIndex eff_idx)
{
    ...
    if (!m_spellInfo->IsTargetInRange(_player, pet))
    {
...
void Spell::EffectSummonCritter(SpellEffectIndex eff_idx)
{
    ...
    if (m_spellInfo->Id == POLEY_DUMMY_SUMMON_SPELL && m_CastItem->GetEntry() != ACTUAL_POLEY_ITEM) { // Original item
        ...
        if (creature_entry) {
            ...
            sLog.outErrorDb("Spell::DoSummonCritter: (custom pet) creature entry not found for item %u.",
                            m_CastItem->GetEntry());
            return;
        }
