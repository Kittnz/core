...
void Spell::CheckAtDelay(TargetInfo* pInf)
{
    ...
    if (pTarget != m_caster && (pTarget->IsImmuneToDamage(m_spellInfo->GetSpellSchoolMask(), m_spellInfo) || pTarget->IsImmuneToSpell(m_spellInfo, pTarget == m_caster)))
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask)
{
    ...
    if (m_caster != unit && m_spellInfo->speed && (unit->IsImmuneToDamage(m_spellInfo->GetSpellSchoolMask(), m_spellInfo) || unit->IsImmuneToSpell(m_spellInfo, unit == realCaster)))
    {
        if (realCaster)
    ...
    if (realCaster && realCaster != unit)
    {
        ...
        else
        {
            ...
            if (m_delayed && !m_spellInfo->IsPositiveSpell() && realCaster->GetMountID() != GNOMECAR_DISPLAYID && realCaster->GetMountID() != GOBLINCAR_DISPLAYID)
            {
                realCaster->SendSpellMiss(unit, m_spellInfo->Id, SPELL_MISS_EVADE);
                ResetEffectDamageAndHeal();
                return;
            ...
            if (m_timer > 0)
            {
                ...
                else if (m_caster->GetTypeId() == TYPEID_UNIT && ((Creature*)m_caster)->IsPet())
                {
                    ...
                    if (m_caster->hasUnitState(UNIT_STAT_CAN_NOT_REACT))
                    {
                        ...
                        if (m_caster->hasUnitState(UNIT_STAT_STUNNED) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_STUN)))
                            cancel();

                        if (m_caster->hasUnitState(UNIT_STAT_CONFUSED) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_CONFUSE)))
                            cancel();

                        if (m_caster->hasUnitState(UNIT_STAT_FLEEING) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_FEAR)))
                            cancel();
                    }
                }
...
void Spell::SendChannelUpdate(uint32 time, bool interrupted)
{
    ...
    if (!time)
    {
        ...
        m_caster->m_Events.AddEventAtOffset(event, (interrupted ? 1 : 1000));
    }
    else if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->IsMounted() && m_caster->GetTypeId() == TYPEID_PLAYER && !m_IsTriggeredSpell && !m_spellInfo->IsPassiveSpell() && !(m_spellInfo->Attributes & SPELL_ATTR_CASTABLE_WHILE_MOUNTED))
    {
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (m_spellInfo->Effect[i])
        {
            case SPELL_EFFECT_DUMMY:
            {
                ...
                else if (m_spellInfo->IsFitToFamilyMask<CF_WARLOCK_LIFE_TAP>() && m_caster)
                {
