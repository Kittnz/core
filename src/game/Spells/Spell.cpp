            ...
            if (m_timer > 0)
            {
                ...
                else if (m_caster->GetTypeId() == TYPEID_UNIT && ((Creature*)m_caster)->IsPet())
                {
                    ...
                    if (m_casterUnit->HasUnitState(UNIT_STAT_CAN_NOT_REACT))
                    {
                        ...
                        if (m_casterUnit->HasUnitState(UNIT_STAT_STUNNED) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_STUN)))
                            cancel();

                        if (m_casterUnit->HasUnitState(UNIT_STAT_CONFUSED) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_CONFUSE)))
                            cancel();

                        if (m_casterUnit->HasUnitState(UNIT_STAT_FLEEING) && !(m_channeled && m_spellInfo->HasAura(SPELL_AURA_MOD_FEAR)))
                            cancel();
                    }
                }
            ...
            case SPELL_EFFECT_DUMMY:
            {
                if (m_spellInfo->SpellIconID == 156) // Holy Shock
                {
                    ...
                    if (m_targets.getUnitTarget() && !m_caster->IsFriendlyTo(m_targets.getUnitTarget()) &&
                       (m_caster->GetDistance2dToCenter(m_targets.getUnitTarget()) > NO_FACING_CHECKS_DISTANCE) && !m_caster->HasInArc(M_PI_F, m_targets.getUnitTarget()))
                ...
                else if (m_spellInfo->Id == 46002 || m_spellInfo->Id == 46010 || m_spellInfo->Id == 46200 || m_spellInfo->Id == 46035 || m_spellInfo->Id == 46001)
                {
                    if (m_caster->ToPlayer()->IsInCombat() || m_caster->ToPlayer()->IsBeingTeleported() || (m_caster->ToPlayer()->GetDeathState() == CORPSE) || m_caster->ToPlayer()->IsMoving())
                    {
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    switch (m_spellInfo->rangeIndex)
    {
        ...
        case SPELL_RANGE_IDX_COMBAT:
        {
            if (target)
            {
                ...
                if ((m_caster->GetDistance2dToCenter(target) > NO_FACING_CHECKS_DISTANCE) && !m_caster->HasInArc(M_PI_F, target))
                    return SPELL_FAILED_UNIT_NOT_INFRONT;
    ...
    if (target && target != m_caster)
    {
        ...
        if (m_caster->IsPlayer() && (m_spellInfo->Custom & SPELL_CUSTOM_FACE_TARGET) &&
           (m_caster->GetDistance2dToCenter(target) > NO_FACING_CHECKS_DISTANCE) && !m_caster->HasInArc(M_PI_F, target))
            return SPELL_FAILED_UNIT_NOT_INFRONT;
    }
