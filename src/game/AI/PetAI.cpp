...
void PetAI::HandleReturnMovement()
{
    ...
    else // COMMAND_FOLLOW
    {
        if (!m_creature->GetCharmInfo()->IsFollowing() && !m_creature->GetCharmInfo()->IsReturning())
        {
            ...
            m_creature->GetCharmInfo()->SetIsReturning(true);
            m_creature->GetMotionMaster()->Clear(false);
            m_creature->GetMotionMaster()->MoveFollow(m_creature->GetCharmerOrOwner(), PET_FOLLOW_DIST, m_creature->IsPet()
                && m_creature->ToPet() && m_creature->ToPet()->getPetType() && m_creature->ToPet()->getPetType() == MINI_PET
                ? 270.0f * (M_PI_F / 180.0f): PET_FOLLOW_ANGLE);
        }
    }
}
