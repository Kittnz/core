...
void Player::HandleFoodEmotes(uint32 diff)
{
    ...
    if (m_foodEmoteTimer <= diff)
    {
        ...
        for (auto itr = auraList.begin(); itr != auraList.end(); ++itr)
        {
            ...
            if ((*itr)->GetSpellProto()->IsAuraAddedBySpell(SPELL_AURA_MOD_REGEN) && (*itr)->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED)
            {
                SendPlaySpellVisual(SPELL_VISUAL_KIT_FOOD);
            }
            if ((*itr)->GetSpellProto()->IsAuraAddedBySpell(SPELL_AURA_MOD_POWER_REGEN) && (*itr)->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED)
            {
...
{
    if ((GetQuestStatus(80255) == QUEST_STATUS_INCOMPLETE) || (GetQuestStatus(80302) == QUEST_STATUS_INCOMPLETE)) // Securing the Supply Lines & Unwanted Eyes
    {
