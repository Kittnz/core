            ...
            if (alreadyDone.find(*i) == alreadyDone.end())
            {
                ...
                for (AuraList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
                {
                    if (((*i)->GetModifier()->m_miscvalue & i_spellProto->GetSpellSchoolMask()) &&
                        (*i)->GetSpellProto()->EquippedItemClass == -1 && (*i)->GetSpellProto()->EquippedItemInventoryTypeMask == 0)
                    {
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit *pVictim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool SpellCasted) const
{
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER && (crit_chance > 0 || IsCreature()) && !pVictim->IsStandingUp())
    {
...
bool Unit::IsSpellBlocked(WorldObject* pCaster, Unit *pVictim, SpellEntry const *spellEntry, WeaponAttackType attackType)
{
...
bool Unit::AddSpellAuraHolder(SpellAuraHolder *holder)
{
    ...
    if (!isAlive() && !aurSpellInfo->IsDeathPersistentSpell() && !aurSpellInfo->IsDeathOnlySpell() && (GetTypeId() != TYPEID_PLAYER || !((Player*)this)->GetSession()->PlayerLoading()))
    {
        delete holder;
        return false;
    }
 ...
 */
uint32 Unit::SpellDamageBonusTaken(WorldObject* pCaster, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, Spell* spell)
{
 ...
 * will be called on each tick for periodic damage over time auras
 */
uint32 Unit::SpellHealingBonusTaken(WorldObject* pCaster, SpellEntry const* spellProto, int32 healamount, DamageEffectType damagetype, uint32 stack, Spell* spell)
{
    float  TakenTotalMod = 1.0f;
...
bool Unit::IsImmuneToSpell(SpellEntry const *spellInfo, bool /*castOnSelf*/)
{
      ...
      && !(spellInfo->AttributesEx & SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY))           // can remove immune (by dispell or immune it)
    {
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for (SpellImmuneList::const_iterator itr = schoolList.begin(); itr != schoolList.end(); ++itr)
            if (!(Spells::IsPositiveSpell(itr->spellId) && spellInfo->IsPositiveSpell()) && (itr->type & spellInfo->GetSpellSchoolMask()))
                return true;
    }
...
bool Unit::IsImmuneToSpellEffect(SpellEntry const* spellInfo, SpellEffectIndex index, bool /*castOnSelf*/) const
{
    ...
    if ((effect == SPELL_EFFECT_HEAL || effect == SPELL_EFFECT_HEAL_MAX_HEALTH) && (GetCreatureType() == CREATURE_TYPE_MECHANICAL))
        return true;
...
void Unit::SetInCombatWithAggressor(Unit* pAggressor, bool touchOnly/* = false*/)
{
    ...
    if (pAggressor->IsPvP())
    {
        if (Player* pThisPlayer = GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            {
                bool valid = true;
                if (pAggressor->GetTypeId() == TYPEID_UNIT)
                {
                    if (pAggressor->isCharmed())
                        valid = false;
                }

                if (valid && pThisPlayer != pAggressorPlayer && !pThisPlayer->IsInDuelWith(pAggressorPlayer) && !(pThisPlayer->IsFFAPvP() && pAggressorPlayer->IsFFAPvP()))
                {
...
bool Unit::isVisibleForOrDetect(WorldObject const* pDetector, WorldObject const* viewPoint, bool detect, bool /*inVisibleList*/, bool* alert) const
{
    ...
    if (invisible || m_Visibility == VISIBILITY_GROUP_STEALTH)
    {
        ...
        if (!pDetector->IsHostileTo(this))
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER && pDetectorPlayer)
            {
