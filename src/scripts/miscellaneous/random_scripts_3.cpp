...
bool GossipSelect_ShopRefundNPC(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
        {
            if (ShopLogEntry* pEntry = sObjectMgr.GetShopLogEntry(shopId))
            {
                if (CanRefundShopItem(pEntry, pPlayer))
                {
                    ...
                    for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
                    {
                        if (Item* pItem = pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
                        {
                            if (pItem->GetTransmogrification())
                            {
                                ...
                                if (tmogProto && tmogProto->SourceItemId == pEntry->itemEntry)
                                    pPlayer->GetTransmogMgr()->RemoveTransmog(pItem);
                            }
                        }
                    }
                    ...
                    {
                        if (Bag const* pBag = (Bag*)pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
                        {
                            for (uint32 j = 0; j < pBag->GetBagSize(); ++j)
                            {
                    ...
                    {
                        if (Item* pItem = pPlayer->GetItemFromBuyBackSlot(i))
                        {
                            if (pItem->GetTransmogrification())
                            {
                                auto tmogProto = sObjectMgr.GetItemPrototype(pItem->GetTransmogrification());
    ...
    {
        shopId = uiAction;
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, pPlayer->GetSession()->GetMangosString(50309), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, pPlayer->GetSession()->GetMangosString(50308), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
