...
/**
 * \brief Portal to the Stormwind Vault script. TODO: This script can probably be replaced by an areatrigger teleport.
 */
struct stormwind_vault_portal final : GameObjectAI
{
    explicit stormwind_vault_portal(GameObject* gameObject) : GameObjectAI(gameObject)
    {
    }

    /**
     * \brief The update function called by the script engine.
     * \param uiDiff The number of milliseconds since the last update interval.
     */
    void UpdateAI(uint32 const uiDiff) override
    {
        if (_updateTimer >= uiDiff)
        {
            _updateTimer -= min(_updateTimer, uiDiff);
        }
        ...
        if (_updateTimer < uiDiff)
        {
            ...
            std::list<Player*> players;
            FindPlayersNearPortal(players);

            for (const auto player : players)
            {
                if (player->isDead())
                {
                    if (player->GetCorpse()->GetMapId() == _stormwindVaultLocation.mapid)
                    {
                        // Player is dead inside the vault.
                        if (player->isDead())
                        {
                    ...
                    {
                        // Player is dead somewhere else.
                        // TODO - replace this with localized text.
                        player->GetSession()->SendNotification("You cannot enter this dungeon while dead.");
                        continue;
                    }
                }
                
                // Dungeon is under-construction, only GMs may enter.
                ...
                // We're ready to teleport the player.
...
private:
    const WorldLocation _stormwindVaultLocation = WorldLocation(35, -1.15f, 44.4f, -25.58f, 1.6f);
    const WorldLocation _portalLocation = WorldLocation(0, -8677.60f, 637.04f, 96.90f, 5.3f);
    const uint32_t _updateFrequency = 1000;
    uint32_t _updateTimer = _updateFrequency;

    /**
     * \brief Resets the update timer.
     */
    ...
    /**
     * \brief Queries whether a player is inside the Stormwind Vault.
     * \param player The player to query.
     * \return True if the player is inside the Stormwind Vault.
     */
    bool PlayerIsInside(const Player* player) const
    {
        return player->GetMapId() == _stormwindVaultLocation.mapid;
    }

    /**
     * \brief Teleports a player to the other side of the portal.
     * \n If a player is outside the vault, they will be teleported inside. If they're inside, they'll be teleported out.
     * \n This function does not verify whether the player is allowed to port (e.g. corpse validation).
     * \param player The player to teleport.
     */
    void TeleportPlayer(Player* player) const
    {
        player->TeleportTo(PlayerIsInside(player)
            ? _portalLocation
            : _stormwindVaultLocation);
    }

    /**
     * \brief Fills the provided list with all players within 1 foot of the portal.
     * \param players The empty list to be populated with players.
     */
    void FindPlayersNearPortal(std::list<Player*>& players) const
    {
        MaNGOS::AnyPlayerInObjectRangeCheck check(me, 1.0f, true, false);
        MaNGOS::PlayerListSearcher searcher(players, check);
