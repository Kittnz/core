...
bool GOHello_go_epl_flying_machine(Player* pPlayer, GameObject* pGo)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, "Up to the Tarnum!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);

        if (pPlayer->GetZoneId() == 1377)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, "Up to the Cenarion Hold!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
    }
